...
PSQL="psql --username=chase --dbname=worldcup --no-align --tuples-only -c"
...
echo "$($PSQL "SELECT CASE WHEN MAX(winner_goals) > MAX(opponent_goals) THEN MAX(winner_goals) ELSE MAX(opponent_goals) END AS "max_goals" FROM games")"
...
echo "$($PSQL "SELECT teams.name FROM teams INNER JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE games.round = 'Eighth-Final' AND games.year = 2014 ORDER BY teams.name")"
